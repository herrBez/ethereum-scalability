...
tree having just the root node. This is worth notice because it allows the nodes
of the network to verify that big data-structures (like the world-state) 
corresponds to the one contained in the blocks' header, by simply comparing
a $256$-bit long hash. Moreover, this feature can be exploited to create 
so-called light nodes, as described in~\autoref{sec:node-types}.
