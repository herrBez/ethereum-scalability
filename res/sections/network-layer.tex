...
encode the messages of the Ethereum's network layer. Inside the UDP payload 
the nodes should insert:
\begin{itemize}
    \item the hash of the juxtaposition of the signature, packet-type and
    packet-data, used to verify the integrity of the packet
    \item the signature, that is used to check the identity of the sender, that
    can be verified by using the node ID, that is used as a public Key.
