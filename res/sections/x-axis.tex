...
blockchain network achieve distributed consensus. As we have seen in
\autoref{sec:consensus:algorithm}, in PoW the truth is determined by heavy
computation done by the miners. In PoS, the consensus depends on the
\emph{stake} confirmed by each validator, which is an economic deposit in the
network's cryptocurrency (ether in this case).
...
has successfully executed, the node becomes one of the validators thus
participating in the voting process of the new block. The consensus algorithm
determines how the validators agree on which block will be the next one.
Different type of PoS can be obtained by varying the consensus algorithm and how
