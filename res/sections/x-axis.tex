...
%network in which the nodes are \emph{servent}. The nodes can run different 
%implementations (e.g., 
%geth\footnote{\url{https://github.com/ethereum/go-ethereum}},
%parity\footnote{\url{https://github.com/paritytech/parity}}, etc.), but all
%of them are functionally equivalent, because adhere to the same protocols.
%%In our analysis we do not consider the possible different node types 
%%(\autoref{sec:node-types}), but only full nodes, that maintain a local copy of
%%the state.
